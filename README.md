...
- **Arc**(Circle L1)도 고를 수 있어요 — 메인넷과 테스트넷 둘 다. 가스도 USDC로 내는 체인이라 «가스용 ETH가 따로 필요한» 문제가 없어요. Arc에선 x402 결제를 지갑이 **직접 체인에 올려요**(정산해 줄 페이실리테이터가 없어도 되는 대신, 결제액과 수수료가 같은 잔액에서 나가요)
...
cd kura
git checkout v0.4.2   # 배포본과 같은 소스. 빼면 개발 중인 최신 코드가 받아져요
