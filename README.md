...
- **자동 시작을 켜 뒀다면**: `brew upgrade`나 재설치는 옛 버전을 지우면서 로그인 항목(`~/Library/LaunchAgents/Kura.plist`)도 같이 내려요. **0.1.1부터는 앱이 이걸 기억했다가 업데이트 뒤 알아서 되살려요.** 다만 **0.1.0 → 0.1.1 이 한 번은 되살릴 수 없어요** — 0.1.0이 "켜 뒀다"는 사실을 어디에도 기록하지 않았거든요. 이번 한 번만 설정에서 다시 켜 주세요.
