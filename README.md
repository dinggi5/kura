...
> **`--no-sign` 이 왜 필요한가요?** 0.1.1부터 앱에 업데이트 공개키가 들어 있어요. 그러면 빌드 도구가 "짝이 되는 개인키를 내놔라"며 멈추는데, 그 키는 배포자만 갖고 있어요. `--no-sign` 은 그 서명 단계를 건너뛰라는 뜻이에요 — 앱은 정상으로 나오고, 직접 빌드한 앱은 자동 업데이트만 안 될 뿐이에요(어차피 직접 빌드해 쓰는 거라면 그게 맞아요). 애플 코드서명도 같이 건너뛰는데, 내 맥에서 내가 빌드한 앱은 Gatekeeper가 안 막아요.

> 태그를 찍어 빌드하면 **같은 소스**임은 보장돼요. 다만 나온 파일이 배포 DMG와 **바이트 단위로 같지는** 않아요(빌드 시각·경로·서명이 들어가요). 재현 빌드는 아직 안 맞춰뒀어요.

### 업데이트

**0.1.1부터는 앱이 스스로 업데이트해요.** 켤 때 새 버전이 있는지 확인하고, 있으면 지갑 화면에 한 줄 띄워요. **설정 → 정보**에서 버전과 바뀐 내용을 확인하고 **지금 설치하고 다시 시작**을 누르면 끝이에요.

- 확인만 자동이고 **설치는 항상 직접 눌러야** 해요. 몰래 바뀌지 않아요.
- 받은 파일은 서명이 맞는지 검사한 뒤에만 설치돼요(서명 검증은 끌 수 없어요). 자세한 건 [SECURITY.md](SECURITY.md)의 "업데이트" 절에 적어 뒀어요.
- 승인 대기 중인 결제가 있으면 설치가 막혀요 — 설치 끝에 앱이 재시작되는데, 그때 대기 중이던 요청이 응답 없이 죽거든요. 요청을 먼저 처리하고 다시 누르세요.
- 확인 통신이 싫으면 **설정 → 정보 → 시작할 때 확인**을 끄세요. 그러면 그 버튼을 직접 누를 때만 확인해요.

**0.1.0에서 0.1.1로 올릴 때만 한 번은 수동이에요** (0.1.0에는 업데이트 기능 자체가 없어요):

- **Homebrew**: `brew upgrade --cask dinggi5/tap/kura`
- **DMG**: [Releases](https://github.com/dinggi5/kura/releases)에서 받아 덮어쓰기

⚠️ **덮어쓰기 전에 Kura를 완전히 종료하세요.** Kura는 창을 닫아도 백그라운드에 남아 있어서, 앱 파일만 바꾸면 **이미 켜져 있는 구버전이 그대로 계속 돌아요.** 보안 수정판을 받아놓고 옛 버전을 쓰는 상황이 생겨요. (앱이 스스로 하는 업데이트는 재시작까지 알아서 하니 이 절차가 필요 없어요.)
...
- **자동 시작을 켜 뒀다면**: `brew upgrade`나 재설치는 옛 버전을 지우면서 로그인 항목(`~/Library/LaunchAgents/Kura.plist`)도 같이 내려요. **0.1.1부터는 앱이 이걸 기억했다가 다음에 켤 때 알아서 되살려요** (업그레이드든 같은 버전 재설치든 똑같이요). 다만 **0.1.0 → 0.1.1 이 한 번은 되살릴 수 없어요** — 0.1.0이 "켜 뒀다"는 사실을 어디에도 기록하지 않았거든요. 이번 한 번만 설정에서 다시 켜 주세요.
