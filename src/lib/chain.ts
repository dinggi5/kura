...
export interface ChainConfig {
  ...
  erc8004: boolean;
  /** **다른 체인의 USDC 를 옮겨 오는 공식 브릿지** (개발 63, 메인넷 전용). 아직 상장 거래소가 없는
   *  체인(Arc 메인넷)은 거래소 입금 안내만으론 채울 길이 없다 — 받기 화면이 이 주소를 한 줄 더 안내한다.
   *  거래소가 생겨도 유효한 길이라 그때 지우지 않는다. 테스트넷은 Faucet 이 그 자리다. */
  bridge?: string;
}
...
export const ARC_MAINNET: ChainConfig = {
  ...
  // 보류(개발 62 「다음」 5번) — 사람이 한 번 채우는 동작이라 링크 한 줄이 맞는 크기다.
  bridge: "https://bridge.usdc.com",
};
