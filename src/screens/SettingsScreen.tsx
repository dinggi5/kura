          ...
          <fieldset disabled={busy} className="flex flex-col gap-3 border-0 p-0 m-0 min-w-0">
            ...
            >
              ...
              {/* Arc 직접 제출의 위험 (개발 68, 사장 결정 09-24) — 가스가 곧 USDC 인 체인에서만 지갑이 x402 를
                  직접 올린다(x402.rs 가 native_is_usdc 로 거른다). 문구는 사장 확정본(구조 + 수수료, 러프하게).
                  경고가 아니라 사실이라 콜아웃 없이 흐린 글씨로. */}
              {chain.nativeIsUsdc && (
                <p className="mt-2.5 text-[11px] leading-relaxed text-[var(--color-ink-300)]">
                  {t(
                    "Arc의 x402 결제는 지갑이 직접 체인에 올리는 구조예요. x402 수수료가 잔액의 USDC에서 나가요.",
                    "On Arc, the wallet submits x402 payments to the chain itself. The x402 fee comes out of your USDC balance.",
                  )}
                </p>
              )}
